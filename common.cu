...
void rdma_context::initialize_verbs(const char *device_name)
{
    ...
    cq = ibv_create_cq(context, 4 * OUTSTANDING_REQUESTS, NULL, NULL, 0); /* create a CQ with place for two completions per request */
    if (!cq) {
    ...
    qp_init_attr.cap.max_send_wr = 2* OUTSTANDING_REQUESTS; /* max of 1 WQE in-flight in SQ per request. that's enough for us */
    qp_init_attr.cap.max_recv_wr = 2* OUTSTANDING_REQUESTS; /* max of 1 WQE in-flight in RQ per request. that's enough for us */
    ...
    rdma_context(tcp_port)
{
    ...
    CUDA_CHECK(cudaMallocHost(&images_target, OUTSTANDING_REQUESTS * IMG_BYTES));
    CUDA_CHECK(cudaMallocHost(&images_reference, OUTSTANDING_REQUESTS * IMG_BYTES));
    CUDA_CHECK(cudaMallocHost(&images_out, OUTSTANDING_REQUESTS * IMG_BYTES));
    ...
    mr_images_target = ibv_reg_mr(pd, images_target, OUTSTANDING_REQUESTS * IMG_BYTES, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
    if (!mr_images_target) {
        perror("ibv_reg_mr() failed for input images");
        exit(1);
    }
    mr_images_reference = ibv_reg_mr(pd, images_reference, OUTSTANDING_REQUESTS * IMG_BYTES, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
    ...
    mr_images_out = ibv_reg_mr(pd, images_out, OUTSTANDING_REQUESTS * IMG_BYTES, IBV_ACCESS_REMOTE_READ);
    if (!mr_images_out) {
